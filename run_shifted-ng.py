...
for data in DATA:
    ...
    rez[data.name] = {}
    inds = list(range(data.num_out))
    inds_id, inds_shifted = [], []
    for lab in range(data.num_out):
        if lab%2 == 0:
            inds_id.append(lab)
        else:
            inds_shifted.append(lab)
    mapping = {ind: ind//2 for ind in inds_id}
    rez[data.name]["id_inds"] = mapping

    X_train_id = [X_train[i] for i in range(len(X_train)) if y_train[i] in inds_id]
    y_train_id = [mapping[y_train[i]] for i in range(len(y_train)) if y_train[i] in inds_id]
    X_test_id = [X_test[i] for i in range(len(X_test)) if y_test[i] in inds_id]
    y_test_id = [mapping[y_test[i]] for i in range(len(y_test)) if y_test[i] in inds_id]
    X_test_shifted = [X_test[i] for i in range(len(X_test)) if y_test[i] in inds_shifted]
    y_test_shifted = [y_test[i] for i in range(len(X_test)) if y_test[i] in inds_shifted]

    print(Counter(y_train_id))
    print(Counter(y_test_id))
    print(Counter(y_test_shifted))
