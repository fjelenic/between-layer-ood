...
GPU = "cuda:0"
FILE = "results.pkl"
UNCERNS = [mu.BLOODQuant(), mu.LeastConfidentQuant()]
DATA = []
...
NUM_ENSAMBLE_EST = 5
LONG_DATA = [
    md.NewsGroupsData().name,
    md.AGNewsData().name,
    md.BigPatentData().name,
    md.FilmGenreData().name,
    md.AmazonReviewData().name,
    md.BigPatent2Data().name,
    md.FilmGenre2Data().name,
    md.AmazonReview2Data().name,
]
...
for data in DATA:
    ...
    for model_name in MODEL_NAMES:
        ...
        model = mm.TransformerClassifier(
            model_name, data.num_out, device=torch.device(GPU)
        )
        criterion = (
            nn.BCEWithLogitsLoss() if data.num_out == 1 else nn.CrossEntropyLoss()
        )
        criterion.to(model.device)
        rez[data.name][model_name]["pre-trained"] = {}
        rez[data.name][model_name]["pre-trained"]["CKA"] = {}
        for X, distrib_type in zip(
            [X_train_id, X_test_id, X_test_ood], ["train", "id", "ood"]
        ):
            ...
            kwargs = {
                ...
                "model": model,
                "criterion": criterion,
                "batch_size": 16 if data.name in LONG_DATA else BATCH_SIZE_INFERENCE,
                "path": True,
            }
        ...
        for seed in range(NUM_SEEDS):
            ...
            for m in range(NUM_ENSAMBLE_EST):
                ...
                model.train_loop(
                    ...
                    if data.name in LONG_DATA
                    else BATCH_SIZE,
                    cartography=False,
                    lr=lr,
                    X_val=X_test_id,
                    y_val=y_test_id if m == 0 else None,
                )
                models.append(model)

            model_orig = mm.TransformerClassifier(
                model_name, data.num_out, device=torch.device(GPU)
            )
            ...
            for uncertainty in UNCERNS:
                ...
                for X, distrib_type in zip(
                    [
                        X_non_val
                        if uncertainty.name == mu.TemperatureScalingQuant().name
                        else X_test_id,
                        X_test_ood,
                    ],
                    ["id", "ood"],
